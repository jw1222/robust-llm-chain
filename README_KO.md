...
기성 라이브러리들이 부분적으로만 해결하는 세 가지 고통:
...
`robust-llm-chain` 은 이 두 timeout 을 분리한다:
...
### 3. Multi-worker 환경에서 라운드 로빈 단편화
다중 worker 배포 (gunicorn × 8, FastAPI workers, Celery 등) 환경에서 대부분의 OSS 라이브러리는 라운드 로빈 인덱스를 *프로세스마다* 유지한다. worker 8개가 각자 프로세스-로컬 인덱스를 들고 있으면, 동시에 8건의 요청이 같은 provider 로 떨어질 수 있다 — cross-vendor 페일오버가 막으려 했던 그 단일 vendor 집중을 그대로 재현한다. `429` 나 `529` 가 한 번 터지면 다른 vendor 로 흡수되는 게 아니라 worker 들이 lockstep 으로 같이 cascade 한다.

`robust-llm-chain` 은 라운드 로빈 인덱스를 worker 간 *공유* — pluggable `IndexBackend` (현재 Memcached, 본인이 구현한 Redis / etcd / DynamoDB 도 가능 — protocol 은 [ARCHITECTURE.md](https://github.com/jw1222/robust-llm-chain/blob/main/ARCHITECTURE.md) 참조) 를 통해 분산한다. 첫 시도 트래픽이 단일 프로세스가 아닌 배포 전체에 퍼진다. backend 자체가 다운되면 라이브러리는 조용히 local index 로 폴백하지 않고 `BackendUnavailable` 을 raise 한다 — fail-closed 동작 자세한 내용은 [Advanced usage](#advanced-usage) 참조.
