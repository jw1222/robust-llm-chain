...
1. **`ProviderSpec` credential masking — covered paths**: `repr(spec)` / `str(spec)` (custom `__repr__` plus `field(repr=False)`), `vars(spec)` / `spec.__dict__` (blocked by `slots=True`), equality / hashing (`compare=False` on credential fields — prevents pytest assertion introspection from printing credentials in failure diffs), and `pickle.dumps(spec)` (custom `__getstate__` strips credentials before serialization). **Not covered** — `dataclasses.asdict(spec)` and `dataclasses.astuple(spec)` traverse all fields unconditionally and therefore expose credentials in plaintext; `pydantic.BaseModel.model_dump()` likewise exposes any `ProviderSpec` it embeds. **For logging or serialization, use `repr(spec)` — never `asdict(spec)`.**
